...
const SeekerProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/seeker/${user.id}`, { withCredentials: true });
    ...
    <div className="max-w-6xl mx-auto p-8 mt-10 text-gray-800 bg-white shadow-lg rounded-lg">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-6">
        <div className="flex items-center space-x-6">
          <img
            src={profile.profile_url || `https://ui-avatars.com/api/?name=${profile.first_name}`}
            alt="Profile"
            className="w-28 h-28 rounded-full object-cover border-2 border-green-500"
          />
          <div>
            <h1 className="text-3xl font-semibold text-green-700">{profile.first_name} {profile.last_name}</h1>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          ...
          <div>
            ...
            <p><FaGraduationCap className="inline text-green-500 mr-2" /><strong>{profile.university_name}</strong> ({profile.passing_year})</p>
            <p className="text-gray-600">{profile.degree_name} in {profile.specialization}</p>
          </div>
          <div>
            <h2 className="text-xl font-semibold text-green-600">Experience</h2>
            <p><FaBriefcase className="inline text-green-500 mr-2" /><strong>{profile.previous_company}</strong> - {profile.previous_job_role}</p>
        ...
        <div className="bg-gray-100 p-6 rounded-lg shadow">
          ...
          <p><FaGraduationCap className="inline text-green-500 mr-2" /><strong>Qualification:</strong> {profile.degree_name}</p>
          <p><FaBriefcase className="inline text-green-500 mr-2" /><strong>Experience:</strong> {profile.years_of_experience} Years</p>
          <p><FaMapMarkerAlt className="inline text-green-500 mr-2" /><strong>Location:</strong> {profile.city_name}, {profile.country_name}</p>

          <h2 className="text-xl font-semibold text-green-600 mt-6">Contact</h2>
          <p><FaEnvelope className="inline text-green-500 mr-2" /><strong>Email:</strong> {profile.email}</p>
          <p><FaPhone className="inline text-green-500 mr-2" /><strong>Phone:</strong> {profile.phone_number}</p>
          <a href={profile.seeker_linkedin_profile} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline block mt-2">
